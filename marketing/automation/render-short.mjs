...
for (const [index, scene] of spec.scenes.entries()) {
  ...
  inputs.push('-f','lavfi','-t',String(duration),'-i',`color=c=${colors[index % colors.length]}:s=1080x1920:r=30`);
  ...
  filters.push(`[${index}:v]${lineFilters.join(',')},fade=t=in:st=0:d=0.22,fade=t=out:st=${Math.max(0.2,duration-0.25)}:d=0.25[v${index}]`);
}
filters.push(`${spec.scenes.map((_,i)=>`[v${i}]`).join('')}concat=n=${spec.scenes.length}:v=1:a=0,format=yuv420p[v]`);
const args = ['-y',...inputs,'-filter_complex',filters.join(';'),'-map','[v]','-c:v','libx264','-preset','medium','-crf','19','-movflags','+faststart',output];
